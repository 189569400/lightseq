...
class FakeTensorQuantFunction(Function):
    ...
    @staticmethod
    def forward(ctx, inputs, amax, num_bits=8, unsigned=False, narrow_range=True, factor=0.005, skip_scale_grad=False):
        # ctx.save_for_backward(inputs, amax)
        outputs, scale = _tensor_quant(inputs, amax, num_bits, unsigned, narrow_range)
        if unsigned:
            outputs += (2.0 ** (num_bits - 1)) - 1.0
        outputs = (outputs * scale).to(inputs.dtype)
        ctx.skip_scale_grad = skip_scale_grad
        if not ctx.skip_scale_grad:
